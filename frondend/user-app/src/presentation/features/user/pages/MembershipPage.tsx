...
import { GetMembershipPlansUseCase } from "../../../../app/useCases/user/getMembershipPlans";

const userRepository = new UserRepository();
const getMembershipPlansUseCase = new GetMembershipPlansUseCase(userRepository);
...
const MembershipPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      setLoading(true);
      try {
        const { plans: fetchedPlans, total } = await getMembershipPlansUseCase.execute({
          page: currentPage,
          limit: plansPerPage,
        });
  ...
  const getDisplayPrice = (plan: MembershipPlan) => {
    ...
    return billingCycle === "monthly" ? monthlyPrice : monthlyPrice * 12 * 0.9;
  };
  ...
  const handlePayment = async (plan: MembershipPlan) => {
    try {
      const response = await userRepository.subscribeToPlan(plan.id);
      ...
      const options = {
        ...
        handler: async (response: any) => {
          ...
          await userRepository.verifyPayment(paymentData);
            ...
            <div className="flex justify-center mb-16">
              <nav className="inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                ...
                <button
                  ...
                  className="relative сорб-inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:bg-gray-200"
                >
