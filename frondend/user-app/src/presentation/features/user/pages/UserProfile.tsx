...
import { useDispatch, useSelector } from "react-redux"; // [Change 1 - Line 2]: Add useSelector
import * as echarts from "echarts";
import Navbar from "../components/Navbar";
import { UserRepository } from "../../../../infra/api/userApi";
import { GetUserProfileUseCase } from "../../../../app/useCases/user/getUserProfile";
import { UpdateUserProfileUseCase } from "../../../../app/useCases/user/updateUserProfile";
import { setAuth, logoutThunk } from "../../../../infra/redux/slices/authSlice"; // [Change 2 - Line 8]: Import setAuth, logoutThunk
import { toast } from "react-toastify";
import { UserProfileData } from "../../../../domain/entities/user/User";
import { UserAuth } from "../../../../domain/entities/common/UserAuth";
import { RootState, AppDispatch } from "../../../../infra/redux/store"; // [Change 3 - Line 11]: Import RootState, AppDispatch
import { useNavigate } from "react-router-dom"; // [Change 4 - Line 12]: Import useNavigate
...
const UserProfile: React.FC = () => {
  const dispatch = useDispatch<AppDispatch>(); // [Change 5 - Line 21]: Use AppDispatch
  const navigate = useNavigate(); // [Change 6 - Line 22]: Add navigate
  const { user } = useSelector((state: RootState) => state.auth); // [Change 7 - Line 23]: Get user from state
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (Object.keys(updateData).length > 0) {
        const user = await updateUserProfileUseCase.execute(updateData);
        setProfileData(user);
        ...
        dispatch(setAuth({ user: authUser, isAuthenticated: true })); // [Change 8 - Line 131]: Use setAuth
  ...
  const handleLogout = async () => {
    try {
      if (user?.email) {
        await dispatch(logoutThunk(user.email)); // [Change 9 - Line 146]: Add logout
